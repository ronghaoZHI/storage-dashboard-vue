<template>
    <div class="bsc-picture-preview">
        <div class="content">
            <div class="close-preview" @click="close">
                <Icon type="close" size="50"></Icon>
            </div>
            <div class="picture-show-box">
                <Spin size="bigger" fix v-if="spinShow"></Spin>
                <img class="box-picture" :src="pictureUrl" @load="adaptPage();spinShow=false;">
            </div>
        </div>
        <div class="right-gallery-list">
            <p class="gallery-list-item" v-for="(item, index) in pictureUrlList" @click="changePicture(item, index)">
                <img class="gallery-list-img" :src="item">
            </p>
        </div>
        <div class="bottom-gallery-details">
            <div class="details-number">{{selectedPictureIndex}} / {{pictureNumber}}</div>
            <div class="details-actions">
                <Tooltip class="actions-icon" content="放大" placement="top">
                    <div @click="larger()">
                        <Icon type="plus" size="28"></Icon>
                    </div>
                </Tooltip>
                <Tooltip class="actions-icon" content="缩小" placement="top">
                    <div @click="smaller()">
                        <Icon type="minus" size="28"></Icon>
                    </div>
                </Tooltip>
                <Tooltip class="actions-icon" content="实际大小" placement="top">
                    <div @click="actualSize()">
                        <Icon type="android-contract" size="28"></Icon>
                    </div>
                </Tooltip>
                <Tooltip class="actions-icon" content="适应页面" placement="top">
                    <div @click="adaptPage()">
                        <Icon type="android-expand" size="28"></Icon>
                    </div>
                </Tooltip>
                <Tooltip class="actions-icon" content="下载原图" placement="top">
                    <div @click="download()">
                        <Icon type="ios-cloud-download" size="28"></Icon>
                    </div>
                </Tooltip>
            </div>
        </div>
...
export default {
    ...
    watch: {
        ...
        'height' (to, from) {
            this.width < $('.picture-show-box, .big-picture-show-box').width() && to < $('.picture-show-box, .big-picture-show-box').height() ? $('.picture-show-box, .big-picture-show-box').addClass('picture-show-box').removeClass('big-picture-show-box') : $('.picture-show-box, .big-picture-show-box').addClass('big-picture-show-box').removeClass('picture-show-box')
        }
    },
    created () {
        // ESC close
    ...
    methods: {
        ...
        larger () {
            ...
            $('.box-picture').width(this.width = width >= 5000 ? width : width * 1.25)
