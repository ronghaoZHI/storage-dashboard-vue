<template>
  <div class="bsc-idc-card">
    ...
    <!--SATA & SSD-->
    <div class="content two-type"
         v-if="type === 'all'">
      <div class="idc-circel idc-content-left">
        <i-circle :percent="info.SATA.used_rate || 0"
      ...
      <div class="idc-circel idc-content-right">
        <i-circle :percent="info.SSD.used_rate || 0"
                  :trail-color="trailColor"
                  :size="80"
                  stroke-color="#FFDE29"
                  :stroke-width="9"
...
export default {
  ...
  computed: {
    ...
    info() {
      ...
      newData.SSD.used_rateFront =
        this.data.SSD.used_rate || this.data.SSD.used_rate === 0
          ? `${this.data.SSD.used_rate}%`
          : '-'
      newData.SATA.used = bytes(this.data.SATA.used)
      newData.SATA.capacity = bytes(this.data.SATA.capacity)
      newData.SATA.used_rateFront =
        this.data.SATA.used_rate || this.data.SATA.used_rate === 0
          ? `${this.data.SATA.used_rate}%`
          : '-'
      return newData
    },
    type() {
