<template>
  <div class="bsc-system bsc-group">
    ...
    <div class="header">
      <div class="search"
           @keyup.enter="getGroupList(false)">
        <Select :prepend="true"
                v-model="searchType"
        ...
        <p class="redFont search-info"
           v-if="searchValueError">{{$t('SYSTEM.GROUP_ID_SEARCH_INFO')}}</p>
      </div>
      <div class="status">
        <span class="group-status-prefix">{{$t('SYSTEM.GROUP_STATUS')}}:</span>
        <button v-bind:class="{statusButtonFocus: read_only === 0}"
                @click="read_only = 0">{{$t('SYSTEM.WRITEABLE')}}</button>
        <button v-bind:class="{statusButtonFocus: read_only === 1}"
                @click="read_only = 1">{{$t('SYSTEM.READ_ONLY')}}</button>
        <button v-bind:class="{statusButtonFocus: read_only === 'ignore'}"
                @click="read_only = 'ignore'">{{$t('SYSTEM.ALL')}}</button>
      </div>
    </div>
    <div class="content">
      <Spin size="bigger"
      ...
      <div class="section-chart-tab">
        <button v-bind:class="{buttonFocus: showChart === 'group_id'}"
                @click="tabToggle('group_id')">{{$t('SYSTEM.CREATE_TIME')}} {{$t('SYSTEM.DESCENDINHG_ORDER')}}</button>
        <button v-bind:class="{buttonFocus: showChart === 'cpu'}"
                @click="tabToggle('cpu')">CPU Load {{$t('SYSTEM.DESCENDINHG_ORDER')}}</button>
        <button v-bind:class="{buttonFocus: showChart === 'ioutil'}"
                @click="tabToggle('ioutil')">IO {{$t('SYSTEM.DESCENDINHG_ORDER')}}</button>
        <button v-bind:class="{buttonFocus: showChart === 'space'}"
                @click="tabToggle('space')">{{$t('SYSTEM.CAPACITY_UTILIZATION_RATE')}} {{$t('SYSTEM.DESCENDINHG_ORDER')}}</button>
        <div class="refresh-section">
          <span @click="getGroupList(false, true)">
            <Icon type="refresh"
                  size="20"></Icon>
          </span>
        </div>
      ...
      <div class="section-chart">
        <div class="card-chart">
          <h1 class="no-data"
              v-if="groupList.length === 0">No Groups</h1>
          <group-card v-for="group in groupList"
                      :key="group.group_id"
                      :data="group"
                      :sortBy="showChart"></group-card>
          <div class="show-more"
               v-show="nextGroupId !== null && nextGroupId !== undefined "
      ...
      list.forEach((group) => {
        ...
        _.forIn(group.partition, (partition) => {
          ...
          if (partition.used_rate > group.space)
            group.space = partition.used_rate
        })
      })
    },
