  ...
  <div class="bsc-system bsc-partition">
    <div class="header">
      <div class="status">
        ...
        <button v-bind:class="{statusButtonFocus: tabName === 'used'}"
                @click="tabToggle('used')">{{$t('SYSTEM.PARTITION_USED')}}</button>
        <button v-bind:class="{statusButtonFocus: tabName === 'unused'}"
                @click="tabToggle('unused')">{{$t('SYSTEM.PARTITION_UNUSED')}}</button>
        <button v-bind:class="{statusButtonFocus: tabName === 'deleted'}"
                @click="tabToggle('deleted')">{{$t('SYSTEM.PARTITION_DELETED')}}</button>
      </div>
      <div class="search"
           v-if="tabName !== 'deleted'"
        ...
        <Select :prepend="true"
          ...
          <Icon slot="prepend"
                size="18"
                type="android-person"></Icon>
          <Option value="ignore"
                  key="ignore">{{$t('SYSTEM.IDC_ALL')}}</Option>
          <Option v-for="idc in idcAll"
                  :value="idc"
                  :key="idc">{{idc}}</Option>
        </Select>
        <Select :prepend="true"
                style="width:180px;margin-right:8px;"
                v-model="search.media_type"
                @on-change="searchList">
          <Icon slot="prepend"
                size="18"
                type="soup-can"></Icon>
          <Option v-for="media in typeArray"
                  :value="media.value"
                  :key="media.value">{{media.name}}</Option>
        </Select>
        <Select :prepend="true"
                style="width:180px;margin-right:8px;"
                v-if="tabName === 'used'"
                v-model="search.read_only"
                @on-change="searchList">
          <Icon slot="prepend"
                size="18"
                type="compose"></Icon>
          <Option v-for="read in readArray"
                  :value="read.value"
                  :key="read.value">{{read.name}}</Option>
        </Select>
        <Select :prepend="true"
                style="width:180px;margin-right:8px;"
                v-if="tabName === 'used'"
          ...
          <Icon slot="prepend"
                size="18"
                type="android-radio-button-on"></Icon>
          <Option v-for="fail in failArray"
                  :value="fail.value"
                  :key="fail.value">{{fail.name}}</Option>
        </Select>
        ...
        <Button type="primary"
                @click="resetParams"
                v-if="tabName === 'used'">{{$t('PUBLIC.RESET')}}</Button>
        <Button type="primary"
                @click="showType = 'card'"
                v-if="showType === 'table'">卡片展示</Button>
        <Button type="primary"
                @click="showType = 'table'"
                v-if="showType === 'card'">列表展示</Button>
      </div>
      <div class="search search-unused"
           v-if="tabName === 'unused'">
        {{$t('SYSTEM.OVERALL_CAPACITY')}}: {{capacity}}
        <span class="title">{{$t('SYSTEM.TOTAL_CAPACITY')}}</span>
        <InputNumber :max="lowerCapacityMax"
                     :min="0"
                     v-model="search.min_capacity"></InputNumber>
        <span>T</span>
        <span class="separate">—</span>
        <InputNumber :min="upperCapacityMin"
                     v-model="search.max_capacity"></InputNumber>
        <span>T</span>
        <span class="title">{{$t('SYSTEM.AVAILABLE')}}</span>
        <InputNumber :max="lowerFreeMax"
                     :min="0"
                     v-model="search.min_free"></InputNumber>
        <span>T</span>
        <span class="separate">—</span>
        <InputNumber :min="upperFreeMin"
                     v-model="search.max_free"></InputNumber>
        <span>T</span>
        <Button type="primary"
                @click="searchList"
                class="search-button">{{$t('SYSTEM.SEARCH')}}</Button>
        <Button type="primary"
                @click="resetParams">{{$t('PUBLIC.RESET')}}</Button>
      </div>
    </div>
    <div class="content">
      <Spin size="bigger"
      ...
      <div v-if="tabName === 'used'">
        <div class="idc-list">
          <idc-card v-for="idc in idcList"
                    :data="idc"
                    :key="idc.idc"></idc-card>
        </div>
        <Table v-if="showType === 'table'"
               border
               :context="self"
               :stripe="true"
               :columns="usedHeader"
        ...
        <Page v-if="showType === 'table' && pageTotal > 1"
              class="page"
              :total="listNum"
              :page-size="20"
        ...
        <div class="section-chart-tab"
             v-if="showType === 'card'">
          <button v-bind:class="{buttonFocus: showChart === 'ioutil'}"
                  @click="chartToggle('ioutil')">IO {{$t('SYSTEM.DESCENDINHG_ORDER')}}</button>
          <button v-bind:class="{buttonFocus: showChart === 'cpu'}"
                  @click="chartToggle('cpu')">CPU Load {{$t('SYSTEM.DESCENDINHG_ORDER')}}</button>
          <button v-bind:class="{buttonFocus: showChart === 'used_rate'}"
                  @click="chartToggle('used_rate')">{{$t('SYSTEM.CAPACITY_UTILIZATION_RATE')}} {{$t('SYSTEM.DESCENDINHG_ORDER')}}</button>
          <div class="refresh-section">
            <span @click="getUsedList">
              <Icon type="refresh"
                    size="20"></Icon>
            </span>
          </div>
        ...
        <div class="section-chart"
          ...
          <div class="card-chart">
            <h1 class="no-data"
                v-if="partitionList.length === 0">No Data</h1>
            <partition-card v-else
                            v-for="partition in partitionList"
                            :data="partition"
                            :detail="usedDetail"
                            :spinGroup="spinGroup"
                            :key="partition.partition_id"></partition-card>
            <br>
            <Page v-if="pageTotal > 1"
                  class="page"
                  :total="listNum"
                  :page-size="20"
                  @on-change="pageChange"
      ...
      <partition-unused v-if="tabName === 'unused'"
                        :data="unusedList"></partition-unused>
      <partition-deleted v-if="tabName === 'deleted'"
                         :data="deletedList"></partition-deleted>
    </div>
    <Page v-if="tabName === 'unused' && pageTotal > 1"
          class="page"
          :total="pageTotal"
          @on-change="pageChange"
          show-elevator></Page>
...
export default {
    ...
    return {
      ...
      usedHeader: [
        ...
        },
        {
          ...
          render: (h, params) => {
            const font =
              params.row.ioutil || params.row.ioutil === 0
                ? `${params.row.ioutil}%`
                : '--'
            return h(
              'div',
        ...
        },
        {
          ...
          render: (h, params) => {
            const font =
              params.row.used_rate || params.row.used_rate === 0
                ? `${params.row.used_rate}%`
                : '--'
            return h(
              'div',
              {
        ...
        },
        {
          ...
          render: (h, params) => {
            const font =
              params.row.cpu || params.row.cpu === 0
                ? `${params.row.cpu}%`
                : '--'
            return h(
              'div',
        ...
        },
        {
          ...
          render: (h, params) => {
            const font =
              (params.row.space || params.row.space === 0) &&
              (params.row.capacity || params.row.capacity === 0)
        ...
        },
        {
          ...
          render: (h, params) => {
            const buttons =
              params.row.readonly === 0
                ? ''
                : params.row.group_num === 0
                  ? h(
                      'i-button',
                      {
                        class: ['delete'],
  ...
  computed: {
    ...
    modalData() {
      ...
      let basicInfo = [
        { name: 'Partition ID', value: this.detailItem.partition_id },
  ...
  methods: {
    ...
    async getUsedList() {
      ...
      try {
        ...
        this.pageTotal = listData.total_count
          ? Math.ceil(listData.total_count / 20)
          : 1
        this.spinContent = false
        this.$Loading.finish()
      } catch (error) {
    ...
    async getUnusedList() {
      ...
      try {
        ...
        const params = {
          ...
          media_type: this.search.media_type,
          ip: this.IPparam,
          min_capacity: this.search.min_capacity
            ? bytes(`${this.search.min_capacity}TB`)
            : 'ignore',
          max_capacity: this.search.max_capacity
            ? bytes(`${this.search.max_capacity}TB`)
            : 'ignore',
          min_free: this.search.min_free
            ? bytes(`${this.search.min_free}TB`)
            : 'ignore',
          max_free: this.search.max_free
        ...
        this.pageTotal = unusedData.total_count
          ? Math.ceil(unusedData.total_count / 20)
          : 1
