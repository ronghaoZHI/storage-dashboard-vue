<template>
  <div class="unused-box">
    <h1 class="no-data"
        v-if="data.length === 0">No Data</h1>
    <p class="page-info"
       v-if="data.length !== 0">{{$t('SYSTEM.UNUSED_INFO')}}</p>
    <div class="bsc-system-card bsc-partition-card"
         v-for="(partition,index) in data"
         :key="partition.idc">
      <div class="header">
      ...
      <div class="content">
        <div class="details">
          <p>
            <span>IDC : </span>{{partition.idc}}</p>
          <p>
            <span>{{$t('SYSTEM.UNUSED_TOTAL_CAPACITY')}} : </span>{{`${bytes(partition.free)}/${bytes(partition.capacity)}`}}</p>
          <p>
            <span>{{$t('SYSTEM.MEDIA_TYPE')}} : </span>{{partition.media_type}}</p>
        </div>
      </div>
      <div class="footer">
        <div>
          <Button type="ghost"
                  size="small"
                  @click="unusedAdd(index)">{{$t('SYSTEM.IMPLEMENTATION')}}
          </Button>
        </div>
      </div>
    </div>
