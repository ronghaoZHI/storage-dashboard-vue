  ...
  <div>
    <div class="layout-bsc-toolbar layout-legend">
      <div>
        <Button class="button-bsc-add-bucket"
                type="primary"
                @click="goJobEdit">{{$t('VIDEO.CREATE_JOB')}}</Button>
        <div class="select-box">
          <span class="quary-name">{{$t('VIDEO.PIPE')}} : </span>
          <Select v-model="pipeId"
                  style="width:200px"
                  @on-change="listJobsByPipeline">
    ...
    <div class="section-paging">
      <Button v-show="pageToken.length > 0"
              @click="listJobsByPipeline(pipeId);pageToken.length = 0"
              type="ghost"
...
const convert2Front = async (data) => {
  ...
  if (data && data.length > 0) {
    ...
    data.forEach((item) => {
      ...
      front.outputNames = item.Outputs
        ? item.Outputs.map((item) => {
            return item.Key
          })
        : ''
      front.templates = item.Outputs
        ? item.Outputs.map((item) => {
            return `${item.PresetId}:${
              templateInfo.templateName[item.PresetId]
            }`
          })
        : ''
