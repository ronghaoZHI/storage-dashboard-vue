      ...
      <div class="editBlock">
        ...
        <div class="form-item">
          <FormItem :label="$t('VIDEO.JOB_PIPE')"
                    prop="PipelineId"
                    required>
            <Select v-model="job.PipelineId"
                    class="line-width"
                    @on-change="pipeChange">
        ...
        <div class="form-item">
          ...
          <div class="section-search">
            <span class="bsc-input">
              ...
              <Button type="text"
                      size="small"
                      @click="getFiles(searchValue)">
                <Icon type="search"
                      :size="iconSize"></Icon>
              </Button>
            </span>
          </div>
          <p class="style-name-info"
             v-if="fileInfo">{{$t('VIDEO.SEARCH_INFO')}}</p>
        </div>
        <div class="form-item">
          <FormItem prop="Inputs">
            <Select v-model="job.Inputs[0].Key"
                    class="line-width">
              <Option v-for="file in fileList"
                      :value="file.Key"
                      :key="file.Key">{{ file.Key }}</Option>
            </Select>
          </FormItem>
        </div>
      </div>
      ...
      <div class="editBlock">
        ...
        <div class="form-item"
          ...
          <Radio-group v-model="job.Playlists.Format">
            <Radio key="HLSv3"
                   label="HLSv3">HLSv3</Radio>
          </Radio-group>
        </div>
        <div class="form-item"
             v-if="MPOpen">
          <FormItem :label="$t('VIDEO.HLS_FILE_NAME')"
                    prop="Playlists.Name"
                    required>
        ...
        <div class="form-item"
          ...
          <FormItem :label="`${$t('VIDEO.OUTPUT_FILE_NAME')} : `"
                    prop="Playlists.OutputKeys"
                    required>
            <Select v-model="job.Playlists.OutputKeys"
                    multiple
                    class="line-width">
              <Option v-for="ots in MPNames"
                      :value="ots"
                      :key="ots">{{ ots }}</Option>
            </Select>
          </FormItem>
      ...
      <Form ref="outputForm"
        ...
        <FormItem :label="$t('VIDEO.TRANSCODING_TEMPLATE') + ' : '"
          ...
          <Select v-model="outputModal.PresetId"
                  ...
                  filterable>
            <Option v-for="template in templateInfo.templateList"
                    :value="template.Id"
...
export default {
  ...
  methods: {
    ...
    editOutput(index) {
      ...
      this.outputModal.StartTime = this.outputModal.Composition
        ? formateSS(this.outputModal.Composition[0].TimeSpan.StartTime)[0]
        : '0:0:0'
      this.outputModal.StartTimeMS = this.outputModal.Composition
        ? formateSS(this.outputModal.Composition[0].TimeSpan.StartTime)[1]
        : '0'
      this.outputModal.Duration = this.outputModal.Composition
        ? formateSS(this.outputModal.Composition[0].TimeSpan.Duration)[0]
        : '0:0:0'
      this.outputModal.DurationMS = this.outputModal.Composition
        ? formateSS(this.outputModal.Composition[0].TimeSpan.Duration)[1]
        : '0'
      this.showOutputsModal = true
      this.HLSError = false
    },
