  ...
  <div class="bsc-pipeline-list">
    <div class="layout-bsc-toolbar layout-legend">
      <Button class="button-bsc-add-bucket"
              type="primary"
              @click="goPipelineEdit('none')">{{$t('VIDEO.NEW_PIPELINE')}}</Button>
    ...
    <div class="section-paging">
      <Button v-show="pageTokenArray.length > 0"
              @click="listPipelines('');pageTokenArray.length = 0"
              type="ghost"
          ...
          render: (h, params) => {
            return h(
              ...
              [
                h(
                  'div',
                  params.row.permission.map((item) =>
                    h(
                      'Tag',
                      {
                        props: {
  ...
  methods: {
    async listPipelines(pageToken) {
      try {
        ...
        pageToken &&
          !this.pageTokenArray.includes(pageToken) &&
    ...
    convert2Front(data) {
      let frontList = []
      data.forEach((item) => {
        const permissions = item.ContentConfig
          ? item.ContentConfig.Permissions
          : [
              {
