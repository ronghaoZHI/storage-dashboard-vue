  ...
  <div class="bsc-edit">
    ...
    <Form :label-width="180"
      ...
      <div class="editBlock">
        ...
        <FormItem class="form-item"
                  prop="InputBucket"
                  :label="$t('VIDEO.INPUT_BUCKET')+' :'">
          <Select v-model="pipeline.InputBucket"
                  class="line-width">
        ...
        <FormItem class="form-item"
                  prop="OutputBucket"
                  :label="$t('VIDEO.OUTPUT_BUCKET')+' :'">
          <Select v-model="pipeline.OutputBucket"
                  class="line-width">
            <Option v-for="bucket in bucketList"
                    :value="bucket"
                    :key="bucket">{{bucket}}</Option>
      ...
      <div class="editBlock">
        <Button class="button-save"
                type="primary"
                @click="pipelineValidate('pipeline')">{{$t('VIDEO.SAVE')}}</Button>
      </div>
    </Form>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    convert2Front(data) {
      ...
      this.pipeline.SuccessCallbackUrl = data.SuccessCallbackUrl
        ? data.SuccessCallbackUrl.split('http://')[1]
        : ''
      this.pipeline.FailureCallbackUrl = data.FailureCallbackUrl
        ? data.FailureCallbackUrl.split('http://')[1]
        : ''
      const permissions = data.ContentConfig
        ? data.ContentConfig.Permissions
        : [
            {
              Grantee: this.username,
    ...
    convert2Save() {
      ...
      saved.ContentConfig.Permissions = _.without(
        [...group, ...user].map((item) => {
