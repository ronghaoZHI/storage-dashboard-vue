<template>
  <div class="bsc-settings">
    ...
    <Tabs size="small"
      ...
      <Tab-pane :label="$t('PUBLIC.ACL')"
        ...
        <table class="table-permission">
          ...
          <tbody>
            <tr v-for="item in GroupACLList"
              ...
              <td>
                <Button style="margin: 0 6px;visibility:hidden;"
                        size="small">
                  <Icon type="ios-plus"
                        :size="iconSize"></Icon>
                </Button>
          ...
          <tbody>
            <tr v-if="isAdd">
              <td>
                ...
                <Button class="new-user-button"
                        @click="isAdd = false"
                        size="small">{{$t("PUBLIC.CANCLE")}}</Button>
              </td>
              <td>
            ...
            <tr v-for="(item,index) in UserACLList"
              ...
              <td>
                ...
                <Checkbox :disabled="owner == item.Grantee.ID"
                          v-model="item.Permission.WRITE">{{$t("STORAGE.WRITE")}}</Checkbox>
              </td>
              <td>
                <Checkbox :disabled="owner == item.Grantee.ID"
        ...
        <Button class="settings-btn"
                ...
                v-else
                disabled
                :title="$t('STORAGE.INVALID_NEW_USER')">{{$t("STORAGE.SAVE_PERMISSIONS")}}</Button>
      </Tab-pane>
      <Tab-pane :label="$t('STORAGE.CORS_CONFIG')"
                name="cors">
        <Button :disabled="rulesNumber===100"
                class="button-add-rule"
                type="primary"
                @click="addCorsRule">{{$t('STORAGE.ADD_RULE')}}</Button>
        <span class="rules-number"
              v-model="rulesNumber">{{$t('STORAGE.RULES_AT_MOST', { rulesNumber: rulesNumber })}}</span>
        <legend-list class="legend-list"
                     :data="legendList"></legend-list>
        <Table border
               :context="self"
               :stripe="true"
               :columns="listCorsHeader"
               :data="corsRulesList"
               :no-data-text="$t('STORAGE.NO_LIST')"></Table>
      </Tab-pane>
      <Tab-pane :label="$t('STORAGE.BACK_SOURCE')"
                name="backSource">
        <back-source v-if="tabName === 'backSource'"></back-source>
      </Tab-pane>
      <Tab-pane :label="$t('SETTINGS.SECURITY_CHAIN')"
                name="whiteList">
    ...
    <Modal v-model="showCorsModal"
           :title="$t('STORAGE.CORS_CONFIG')"
           width="700"
           class="edit-modal">
      <div class="form-item">
        ...
        <p class="style-error-info redFont"
           v-if="AllowedOrigins">{{$t('STORAGE.PRESS_ENTER_KEY')}}</p>
        <p class="info">{{$t('STORAGE.ENTER_KEY')}}</p>
        <div class="tag-margin-left">
          <Tag type="border"
               color="blue"
               v-for="item in corsModal.AllowedOrigins"
               :key="item"
               :name="item"
               closable
               @on-close="deleteCorsModalTag('AllowedOrigins',item)">{{item}}</Tag>
        </div>
      </div>
      <div class="form-item">
        <span class="form-label required-item">Allowed Methods : </span>
        <Checkbox-group style="display:inline"
                        v-model="corsModal.AllowedMethods">
          <Checkbox class="modal-checkbox"
                    label="GET">GET</Checkbox>
          <Checkbox class="modal-checkbox"
                    label="PUT">PUT</Checkbox>
          <Checkbox class="modal-checkbox"
                    label="HEAD">HEAD</Checkbox>
          <Checkbox class="modal-checkbox"
                    label="POST">POST</Checkbox>
          <Checkbox class="modal-checkbox"
                    label="DELETE">DELETE</Checkbox>
        </Checkbox-group>
        <p class="style-error-info redFont"
           v-if="methodsError">{{$t('STORAGE.SELECT_ONE_AT_LEAST')}}</p>
      </div>
      <div class="form-item">
        ...
        <div class="tag-margin-left">
          <Tag type="border"
               color="blue"
               v-for="item in corsModal.AllowedHeaders"
               :key="item"
               :name="item"
               closable
               @on-close="deleteCorsModalTag('AllowedHeaders',item)">{{item}}</Tag>
        </div>
      </div>
      <div class="form-item">
        ...
        <div class="tag-margin-left">
          <Tag type="border"
               color="blue"
               v-for="item in corsModal.ExposeHeaders"
               :key="item"
               :name="item"
               closable
               @on-close="deleteCorsModalTag('ExposeHeaders',item)">{{item}}</Tag>
        </div>
      </div>
      <div class="form-item">
      ...
      <div slot="footer">
        <Button type="primary"
                @click="putBucketCors">{{$t('VIDEO.OK')}}</Button>
      </div>
    </Modal>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async deleteCorsRule(index) {
      ...
      try {
        this.$Loading.start()
        this.corsRulesList.length !== 0
          ? await handler('putBucketCors', putParams)
          : await handler('deleteBucketCors', { Bucket: this.bucket })
        this.rulesNumber = this.rulesNumber - 1
        this.$Loading.finish()
