<template>
  <div>
    ...
    <link-table :listData="whiteList"
                formName="whiteForm"
                :info="$t('SETTINGS.WHITE_LIST_INFO')"></link-table>
    ...
    <link-table :listData="blackList"
                formName="blackForm"
                :info="$t('SETTINGS.BLACK_LIST_INFO')"></link-table>
  </div>
</template>
<script>
  ...
  list.forEach((testItem) => {
    for (let j = 0; j < 4; j++) {
      ...
      list.forEach((item) => {
        ...
        if (
          itemIP === testIP &&
          item.upload === testItem.upload &&
          item.delete === testItem.delete &&
          item.download === testItem.download
        ) {
