...
const acl2save = (item) => {
  ...
  if (item.GranteeOrigin) {
    ...
    savedItem.Grantee = item.Grantee.includes('@')
      ? { Type: 'AmazonCustomerByEmail', EmailAddress: item.Grantee }
      : { Type: 'CanonicalUser', ID: item.Grantee }
...
const convertGrants = (grants) => {
  let userACLItems = []
  let gropItemsDefault = _.cloneDeep(groupACLListDefult)
  if (grants.length) {
    let IDArry = []
    _.each(grants, (grant) => {
      if (
...
const convertObject2String = (object) => {
  ...
  _.each(object, function(value, key) {
    if (value && key !== 'Write') {
      truePermission.push(acc2per[key])
    }
  })
