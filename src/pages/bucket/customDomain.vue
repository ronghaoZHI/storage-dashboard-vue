<template>
    <div>
        <Button type="primary" @click="createModal = true">{{$t('SETTINGS.BIND_CUSTOME_DOMAIN')}}</Button>
        <p class="page-info">{{$t('SETTINGS.DOMAIN_INFO')}}</p>
        <Table border :stripe="true" :columns="listHeader" :data="listData" :no-data-text='noDataText'></Table>
...
export default {
    ...
    methods: {
        ...
        deleteConfirm (row) {
            this.$Modal.confirm({
                content: this.$t('SETTINGS.DOMAIN_DELETE_CONFIRM', {domain: row.domain}),
        ...
        async deleteDomain (row) {
            try {
                await this.$http.delete(`${CUSTOM_DOMAIN}/${this.bucket}`, {params: {custom_domain: row.domain}})
