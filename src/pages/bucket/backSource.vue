<template>
  <div class="bsc-back-source">
    <Button class="button-add-rule"
            type="primary"
            @click="show404ModalFunc()">{{$t('STORAGE.ADD_RULE')}}</Button>
    ...
    <Table border
           :context="self"
           :stripe="true"
           :columns="list404Header"
           :data="fetchRuleList"
           :no-data-text="$t('STORAGE.NO_LIST')"></Table>
    <Modal v-model="show404Modal"
           :title="$t('STORAGE.BACK_SOURCE')"
           width="650"
           class="edit-modal">
      <Form ref="formValidate404"
            :model="formValidate404"
            :rules="ruleValidate404"
            :label-width="150">
        <FormItem :label="$t('STORAGE.SOURCE_MODE')"
                  prop="fetch_mode">
        ...
        <FormItem v-show="formValidate404.fetch_mode !== 'fetch_302'"
                  :label="$t('STORAGE.SOURCE_HEADER')"
                  prop="source_header">
          ...
          <p class="style-error-info redFont"
             v-if="requestHeader && !requestHeaderFormatError">{{$t('STORAGE.PRESS_ENTER_KEY')}}</p>
          <p class="info">{{$t('STORAGE.ENTER_KEY')}}</p>
          <div class="tag-margin-left">
            <Tag type="border"
                 color="blue"
                 v-for="item in formValidate404.request_headers"
                 :key="item"
                 :name="item"
                 closable
                 @on-close="deleteHeaderTag(item)">{{item}}</Tag>
          </div>
        </FormItem>
        ...
        <FormItem :label="$t('STORAGE.URI_PATTERN')"
                  prop="uri_pattern">
        ...
        <FormItem :label="$t('STORAGE.ALLOW_METHOD')"
                  prop="allow_method">
      ...
      <div slot="footer">
        <Button type="primary"
                @click="validate404Rule('formValidate404', isEdit)">{{$t('VIDEO.OK')}}</Button>
      </div>
    </Modal>
  </div>
</template>
  ...
  methods: {
    ...
    validate404Rule(name, isEdit) {
      this.$refs[name].validate((valid) => {
        valid
          ? this.add404Rule(isEdit)
          : this.$Message.error(this.$t('STORAGE.FORM_FAIL'))
      })
    },
    async add404Rule(isEdit) {
      ...
      if (
        ...
        this.formValidate404.request_headers.length > 0
      ) {
        this.formValidate404.request_headers.forEach((value) => {
          let index = value.indexOf(':')
          rule.request_headers[value.slice(0, index).trim()] = value
            .slice(index + 1)
            .trim()
        })
      }
      try {
        ...
        this.$Loading.error()
        !isEdit
          ? this.$Message.error(this.$t('STORAGE.CREATE_FAIL'))
          : this.$Message.error(this.$t('STORAGE.SET_FAIL'))
      }
    },
    ...
    edit404Rule(row) {
      this.show404Modal = true
      this.formValidate404.fetch_mode = this.fetchRuleList[
        row._index
      ].fetch_mode
      ...
      this.formValidate404.allow_method = this.fetchRuleList[
        row._index
      ].allow_method.split(',')
      this.isEdit = true
      this.editId = row.id
