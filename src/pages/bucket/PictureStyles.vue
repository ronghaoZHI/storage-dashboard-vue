<template>
  <div>
    ...
    <div class="toolbar-nav">
      <Button type="primary"
              @click="goCreateStyle">{{$t("STORAGE.CREATE_STYLE")}}</Button>
      <Button type="primary"
              @click="uploadModal">{{$t("STORAGE.IMPORT_STYLE")}}</Button>
    </div>
    <Table border
           ...
           :columns="styleHeader"
           :data="styleList"
           :no-data-text="$t('STORAGE.NO_STYLE')"></Table>
    ...
    <Modal v-model="showUploadModal"
           title="Upload settings files"
           width="700">
      <upload accept="application/json"
              :prefix="prefix"
              :bucket="bucket"
              :checkFileType="true"
              :validateMessage="$t('STORAGE.PIC_UPLOAD_INFO')"
  ...
  methods: {
    ...
    json2instruction(jsonData) {
      ...
      _.each(jsonData, (value, key) => {
        let item = ''
        if (key === 'overlay') {
          overlayFileName = /^text:.*/.test(value)
            ? /^text:(.*):(.*)/.exec(value)[1] + '.json'
            : value + '.png'
        }
